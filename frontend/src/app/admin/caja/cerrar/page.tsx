...
function CajaCerrarInner() {
  const { token } = useAuth();
  const searchParams = useSearchParams();
  const cashBook = searchParams.get('cashBook') === 'dra' ? 'dra' : 'escuela';
  const [data, setData] = useState<{
    session: { id: string; date: string; opening_amount: number; cash_book?: string; opened_by_name?: string | null };
    totalIncome: number;
    totalExpense: number;
    balance: number;
  } | null>(null);
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`${API_URL}/api/admin/cash/summary?cashBook=${cashBook}`, { headers: getAuthHeaders(token) })
  ...
  if (!data) {
    return (
      <div className="space-y-4">
        <p className="text-neutral-600">No hay caja abierta para cerrar ({cashBook === 'dra' ? 'DRA' : 'Escuela'}).</p>
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-neutral-900">Cerrar caja ({cashBook === 'dra' ? 'DRA' : 'Escuela'})</h2>
      ...
      <div className="rounded-2xl border border-neutral-200 bg-white p-6 space-y-4">
        <div className="flex justify-between text-sm">
          <span className="text-neutral-600">Monto inicial</span>
          <span className="font-semibold">{formatMoney(Number(session.opening_amount))}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-neutral-600">Ingresos (operativos)</span>
          <span className="font-semibold text-emerald-600">+ {formatMoney(totalIncome)}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-neutral-600">Egresos (operativos)</span>
          <span className="font-semibold text-rose-600">− {formatMoney(totalExpense)}</span>
        </div>
        <hr className="border-neutral-200" />
        <div className="flex justify-between text-base">
          <span className="font-medium text-neutral-900">Balance al cierre</span>
          <span className="font-bold text-neutral-900">{formatMoney(balance)}</span>
        </div>
      </div>
      ...
      <button
        ...
        onClick={handleClose}
        disabled={submitting}
        className="w-full rounded-xl bg-red-600 py-3 text-white font-semibold hover:bg-red-700 disabled:opacity-60"
      >
